    ...
    if (params.ch === '\n') {
        ...
        if (previousLine.trimRight().endsWith(':')) {
            ...
            if (subLine.trimRight().length === 0) {
                ...
                return [
                    vscode_languageserver_1.TextEdit.del(vscode_languageserver_1.Range.create(position, vscode_languageserver_1.Position.create(position.line, currentLine.length - 1))),
                    vscode_languageserver_1.TextEdit.insert(position, ' '.repeat(params.options.tabSize + (isInArray ? 2 - indentationFix : 0))),
                ];
            }
            if (isInArray) {
